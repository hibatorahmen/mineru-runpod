...
| Batch (`POST /extract/task/batch`, `file-urls/batch`) | not offered — submit tasks individually and raise `workers_max`; RunPod's queue runs them in parallel across workers ([Scaling](/mineru-runpod/guides/scaling/)) |
