...
**Fix**: this template currently keeps Hub metadata under `.runpod-disabled/` rather than `.runpod/`, so Hub auto-detection is disabled. When the folder is restored, keep `tests_.json` (underscore suffix) rather than `tests.json` if you want to skip validator execution. Same pattern as `runpod-workers/worker-comfyui`.

To re-enable testing later, rename `.runpod-disabled/` back to `.runpod/`, then rename `tests_.json` to `tests.json` and consider switching `gpuTypeId` from `"NVIDIA RTX A5000"` to `"NVIDIA GeForce RTX 4090"` — 4090 has the highest pool availability across RunPod's regions.
